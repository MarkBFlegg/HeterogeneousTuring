...
b_vals = linspace(0, 1 - 1.4901e-08, 100);
...
for betaI = 82:82
    ...
    p = makeP(beta0, 2, sprintf('crit_eps_%i', betaI));
    crit_eps_vals(betaI) = solveCritEps(p, 0.99*getCritEps(1, beta0, 1/40, pi*pi));
end
...
function p = makeP(beta0, eps, fname)
    ...
    opts.par.betaf = @(x,theta) beta0 - (1 - theta *cos(2*pi .* (x+0.5)))*1.4901e-08;
